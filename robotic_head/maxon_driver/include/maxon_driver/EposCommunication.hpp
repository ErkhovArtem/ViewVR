...
#include "maxon_driver/WinTypes.h"
...
class EposCommunication
{
 public:
  ...
  int 	initialization(unsigned short NodeId, float gear_ratio, int offset, bool IsSlave, void** p_pKeyHandle);
 ...
 private:
  ...
  void  PrintHeader();
  void  PrintSettings();
  void  SetDefaultParameters(unsigned short NodeId);
  ...
  int   OpenDevice(void** p_pKeyHandle, unsigned int* p_pErrorCode);
  ...
  int SetInputConfiguration(unsigned int* pErrorCode);
  ...
  float CountsToDegrees(int* counts);
  int 	DegreesToCounts(float deg);
